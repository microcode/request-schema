...
class Schema {
    ...
    on(method, path, ...args) {
        ...
        try {
            data.add_entry(new Entry(filters, func), ['data', 'context'].concat(this._options.extra).concat(queryArgs));
        } catch (err) {
            ...
            try {
                ...
                const args = argNames.map(name => {
                    switch (name) {
                        ...
                        default: {
                            if (capture.has(name)) {
                                return decodeURIComponent(capture.get(name));
                            }

                            if(queryArgs && queryArgs.has(name)) {
                                return decodeURIComponent(queryArgs.get(name));
                            }

                            log.warn('Path "%s" ("%s") is missing argument value for "%s"', path, method, name);
